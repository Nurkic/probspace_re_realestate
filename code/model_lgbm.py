...
prep_train_label = predata_label.iloc[:285931, :]
prep_test_label = predata_label.iloc[285931:, :]
#print(len(train), len(prep_train_label))
""" define data"""
#train_X = prep_train_label.drop(["id", "Prefecture", "Municipality"], axis=1)
#train_y = np.log1p(df)
test_X = prep_test_label.drop(["id", "Prefecture", "Municipality", "y"], axis=1)
...
#print(prep_train_label["y"].isnull().values.sum())
#print(prep_train_label["Period"].value_counts())
def objective(trial):
    params = {
        'task': 'train',
        'boosting_type': 'gbdt',
    ...
    for va_period in va_periods:
        ...
        #print(y_train.isnull().values.sum(), print(y_valid.isnull().values.sum()))
        #print(y_train)
